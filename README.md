...
The app opens on a **demo casino** — an irregular, two-floor facility where each floor combines gaming and back-of-house on one map. A curved serpentine main aisle (the customer route, with no right angles) winds past a table-games strip with slot banks behind it, enclosed Poker and High-Limit rooms, a rounded Bar and Cage, and a Sportsbook, then extends into a service spur reaching the rectangular back-of-house (Count Room, Vault, Surveillance, Security). It's pre-fitted with ~91 cameras by casino doctrine, giving ~96% mean room coverage per floor. Switch floors and toggle **Coverage** / **Blind spots** to see the occlusion engine at work. All edits persist automatically to the browser's local storage; if you've edited a previous building, use **Data → Reset building** to load the demo.
