...
## Open questions — RESOLVED 2026-07-17 (decisions locked for the build; user reviews at final test)

- **OQ-1 — roll and coverage: DECIDED, accept permanently.** Roll never affects the 2D footprint. The visibility polygon is a range-capped angular sweep; a rolled rectangle's floor intersection differs only at the corner fringe, below the fidelity 2D coverage claims. `rollDeg` affects only the rendered frustum/feed orientation.
- **OQ-2 — `vfovDeg`: DECIDED.** Derived at 16:9 unless overridden. Domes ignore `vfovDeg` and render a hemisphere gizmo, not a pyramid. **Derivation divergence — DECIDED, keep it, document it (same reasoning as OQ-6):** the 3D scene (`deriveVfovDeg`, scene-build.ts) uses the exact projection trig above; the 2D tilt band (`vfovHalfRad`, coverage.ts) keeps its shipped linear approximation clamped to [15°, 60°]. For a 90° camera that's 58.7° vs 50.6° — at grazing tilts the 2D band's far edge and the 3D frustum's floor footprint visibly disagree. Unifying would silently reshape every shipped coverage band and report; instead the 3D walk view is the physically-honest preview and the 2D engine remains the authoritative planning/report surface. Both derivations clamp into a sane (0°, 180°) window (wide wedges 180 < fovDeg < 360 and hand-edited overrides are representable and reach the math unvalidated).
- **OQ-3 — walk-under threshold: DECIDED.** A structure occludes 2D sightlines iff `baseM < 1.8` (exported constant `WALK_UNDER_M` in coverage.ts, documented as an approximation the 3D depth view supersedes).
- **OQ-4 — 2D renders structures: DECIDED, yes.** Fill + outline layers, a fill-extrusion entry for the Phase-A 3D view, a `structures` layer-visibility row, and a column-placement tool in the 2D editor. render.ts changes ⇒ viewer template regeneration (accepted).
- **OQ-5 — camera representation in 3D: DECIDED.** Abstract gizmo: small mount body + wireframe frustum + aim line (hemisphere for domes). A modeled camera mesh is game-feel polish, deferred.
- **OQ-6 — corridor edges as occluders: DECIDED, keep the divergence, document it.** 2D `collectWalls` keeps treating every unit edge as a blocker — the shipped coverage numbers (the casino was planned to 99% against these semantics), blind-spot analysis, and security reports all depend on it, and silently reshaping them is worse than the divergence. The 3D walk mode's live coverage is labeled a **physically-honest line-of-sight preview**; the 2D engine remains the authoritative planning/report surface. Revisit only if operators find the disagreement confusing in practice.
