...
// Pre-inlined, committed viewer template (Phase C packaging) — regenerated by
// `npm run build:viewer` (vite.viewer.config.ts + scripts/build-viewer-template.mjs).
// Imported dynamically inside exportViewer() (below), not statically here —
// it's ~1MB and only ever needed at export time, so a static top-level import
// would bloat the main app's initial bundle for a feature most sessions never
// touch. Raw string import (see vite/client.d.ts for the `*?raw` ambient
// module type, referenced via src/vite-env.d.ts).
import type {
  Building,
  MetreXY,
  Category,
...
import { toVisitorBuilding } from "./viewer/visitor";
import {
  buildingFileText,
  buildingKey,
...
interface State {
  ...
  /** Single-file shareable viewer (Phase C): visitor-safe building
   *  (toVisitorBuilding) embedded into the committed, pre-inlined viewer
   *  template. Downloads ONE self-contained .html that opens from file://
   *  with zero network — no editing chrome, no security/operator data. */
  exportViewer: () => Promise<void>;
...
export const useStore = create<State>((set, get) => {
  ...
  return {
    ...
    // Single-file shareable viewer (Phase C, final): toVisitorBuilding runs
    // HERE, at export time, so the JSON embedded in the downloaded file is
    // already stripped of operator/security data by construction — the
    // viewer app itself never re-derives it. `<` is escaped to a unicode
    // escape (not HTML-escaped) so the payload can't prematurely close the
    // wrapping <script id="building-data"> tag if it ever contains something
    // that looks like a tag (e.g. a room named "<script>").
    exportViewer: async () => {
      const s = get();
      const payload = buildingFileText(toVisitorBuilding(s.building), s.propertyId).replace(
        /</g,
        "\\u003c",
      );
      const { default: viewerTemplate } = await import("./generated/viewer-template.html?raw");
      const html = viewerTemplate.replace("/*__BUILDING__*/", () => payload);
      const url = URL.createObjectURL(new Blob([html], { type: "text/html" }));
      const a = document.createElement("a");
      a.href = url;
      a.download = `${s.propertyId}-viewer.html`;
      a.click();
      URL.revokeObjectURL(url);
      set({ importMsg: `Exported shareable viewer (${s.propertyId}-viewer.html).` });
    },
